class Menu():
  ...
  def __repr__(self):
    return f"This is the {self.name} menu, available from {self.start_time}:00 to {self.end_time}:00"

  def calculate_bill(self, purchased_items):
    total_bill = 0
    # For each item in the purchased items
    for item in purchased_items:
      # Add the value of the item to the total bill
...
our_business = Business("Basta Fazoolin' with my Heart", [flagship_store, new_installment])
